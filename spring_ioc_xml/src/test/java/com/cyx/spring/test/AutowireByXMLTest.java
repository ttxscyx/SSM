...
/*
* 自动装配：
* 根据指定的策略,在IOC容器种匹配某个bean，自动为bean中的类类型的属性或接口类型的属性赋值
* 可以通过bean标签中的autowire属性设置自动装配的策略
* 自动装配的策略：
* 1、no/default:表示不装配，即bean中属性不会自动匹配某个bean为属性赋值，此时属性使用默认值
...
* 3、byName：将要赋值的属性的属性明作为bean的id在IOC容器中匹配某个bean，为属性赋值
